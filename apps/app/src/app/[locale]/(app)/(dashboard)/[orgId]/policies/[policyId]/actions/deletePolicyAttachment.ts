...
export const deletePolicyAttachment = async (input: z.infer<typeof schema>) => {
	...
	try {
		...
		attachmentToDelete = await db.attachment.findUnique({
			where: {
				...
				entityType: AttachmentEntityType.policy,
			},
		});
		...
		// Revalidate the policy path if needed, depends on how attachments are loaded
		revalidatePath(
			`/${organizationId}/policies/${attachmentToDelete.entityId}`,
		);
